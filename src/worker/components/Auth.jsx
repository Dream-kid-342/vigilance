...
export default function RoleSignup({ onSelect, initialRole = null, onBack }) {
  ...
  const [mode, setMode]   = useState(initialRole ? 'signup' : 'login');
  const [step, setStep]   = useState(initialRole ? 2 : 1);
  const [role, setRole]   = useState(initialRole || 'worker');
    ...
    <div style={pageStyle}>
      <div className="mobile-px mobile-py" style={{ ...cardStyle, boxSizing: 'border-box' }}>
        ...
        <p style={{ textAlign: 'center', color: t.textMuted, fontSize: '0.83rem', marginTop: '1rem' }}>
          ...
          <button onClick={() => { setMode('signup'); setStep(2); setRole('worker'); setError(null); }} style={{ background: 'none', border: 'none', color: t.primary, cursor: 'pointer', fontWeight: 600, fontFamily: 'inherit', fontSize: '0.83rem' }}>
            Create account
          </button>
        </p>
      </div>
