...
export default function RoleSignup({ onSelect, initialRole = null, onBack }) {
  const { theme: t, isDark } = useTheme();
  // If initialRole is provided, skip step 1 (role selection) and go straight to form
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      await supabase.from('profiles').upsert({
        ...
        role: role, phone_number: form.phone, national_id: form.nationalId,
  ...
  const renderLogin = () => (
    <div style={pageStyle}>
      <div className="mobile-px mobile-py" style={{ ...cardStyle, boxSizing: 'border-box' }}>
        ...
        <p style={{ color: t.textMuted, fontSize: '0.85rem', marginBottom: '1.5rem' }}>Sign in to your Vigilance account</p>
        ...
        <p style={{ textAlign: 'center', color: t.textMuted, fontSize: '0.83rem', marginTop: '1rem' }}>
          ...
          <button onClick={() => { setMode('signup'); setStep(initialRole ? 2 : 1); setRole(initialRole); setError(null); }} style={{ background: 'none', border: 'none', color: t.primary, cursor: 'pointer', fontWeight: 600, fontFamily: 'inherit', fontSize: '0.83rem' }}>
            Create account
          </button>
        </p>
      </div>
  ...
  // Default: if mode forced to signup but initialRole already set, skip to step 2 automatically
